...
const { initializeWalletMappingService } = require('../services/walletMappingService');
...
const logCommandRegistration = (commandName, commandData) => {
  try {
    ...
    console.error(`Error logging command registration for ${commandName}:`, error);
  }
};
...
Promise.all([
  ...
  .then(async ([storageSuccess, quizDbSuccess, pollDbSuccess, walletMappingSuccess]) => {
    if (storageSuccess) {
    ...
    if (quizDbSuccess) {
      ...
      console.warn('Quiz database initialization failed, quiz tracking features will use in-memory storage');
    ...
    if (pollDbSuccess) {
      ...
      console.warn('Poll database initialization failed, poll features will use in-memory storage');
      // Fallback to in-memory storage if database fails
